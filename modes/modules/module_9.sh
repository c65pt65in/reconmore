...
sudo -u $SUDO_USER $path --output-only-positives --browser-cluster-pool-size 6 --browser-cluster-ignore-images $url --report-save-path /dev/null --checks=csrf,code-injection*,ldap_injection,path_traversal,file_inclusion,response_splitting,os_cmd-injection*,rfi,unvalidated_redirect*,xpath_injection,xss*,source_code_disclosure,xxe,backup_files,backup_directories,common_admin_interfaces,common_directories,http_put,unencrypted_password_forms,webdav,xst,cvs_svn_users,htaccess_limit,interesting_responses,html_objects,mixed_resource,insecure_cookies,http_only_cookies,password_autocomplete,form_upload,cookie_set_for_parent_domain --scope-directory-depth-limit 5 2>/dev/null | tee -a $dir/report 
...
then
echo -e "\e[31mCookie set for parent domain: In most situtations it is recommended to omit the 'Domain' attribute in cookies!\e[0m" | tee -a $dir/report
fi
